...
Of the 30 authors sampled under the scheme described in Subsection~\ref{subsec:methods-crossval}, Table~\ref{tab:crossval} summarizes agreement between OpenAlex's \texttt{h\_index} and the corresponding value I located by hand in Web of Science, Scopus, and Google Scholar. Coverage itself varies sharply: Scopus returned a value for 26 of the 30 authors, Google Scholar for 19, and Web of Science for 16.
...
Splitting h\textsubscript{2} by (institution, field) reveals that the overall ranking is deceptive: several institutions that never come close to the top of Table~\ref{tab:overall-h2} rank first globally in their field by this measure. Table~\ref{tab:field-leaders} lists a sample of these cases.
...
University of Antwerp is the counterexample that proves the rule holds both ways: it is \#14 overall ($\mathrm{h}_2=98$) \emph{and} \#1 in Physics and Astronomy ($\mathrm{h}_2=96$), a broad powerhouse that also ranks first in a specific, non-biomedical field. Universidade Estadual de Campinas (UNICAMP) places 6th in Dentistry behind São Paulo, placing Brazil among the top six globally in Dentistry by this measure despite the country appearing nowhere near the top of any general-purpose ranking~\citep{vaccaro2022bibliometrics}. China's cluster is not one specialty but four related applied-science fields: University of Science and Technology of China (USTC) ranks first in Materials Science, Engineering, and Energy by field h\textsubscript{2}, while its sister institution, the University of Chinese Academy of Sciences, ranks first in Chemical Engineering.
...
\subsection{Robustness of field-specific rankings to multi-field assignment}
\label{subsec:results-field-robustness}
The single-field assignment described in Subsection~\ref{subsec:author-assignment} asks which field accounts for the plurality of an author's publication volume, and assigns them exclusively there. To test how much this choice moves the field-level rankings, I re-run the field-specific h\textsubscript{2} computation after allowing each author to count in every field for which their OpenAlex topic-weight share is at least $\tau$ in accordance with Subsection~\ref{subsec:methods-field-robustness}.

At $\tau = 33\%$ (an author must contribute at least one third of their output to count in a field), the (institution, field) pair count rises by 9.3\% (333{,}132 $\to$ 364{,}032 pairs). The Spearman rank correlation between original and multi-field h\textsubscript{2} values, computed on the inner join of shared pairs, is $\rho = 0.963$: field rankings are highly stable. Of the 27 fields' leading institutions\textcolor{red}{ (26 fields, but Social Sciences has two institutions tied for the top h\textsubscript{2} value, yielding 27 leader entries)}, 23 are unchanged. The Wageningen Ag\&Bio leadership (Table~\ref{tab:field-leaders}) survives and Carnegie Mellon's Computer Science lead survives. The one notable exception is Environmental Science: the University of Colorado Boulder overtakes Wageningen at this threshold, because Boulder has a larger body of researchers who do substantial but not modal Environmental Science work. The two methods answer different questions. The single-field assignment identifies institutions where Environmental Science is researchers' \emph{primary} field; the multi-field assignment identifies institutions with the most researchers for whom it is a \emph{substantial} field.

At $\tau = 20\%$ (a more aggressive threshold admitting all authors with one fifth or more of their output in a field), the pair count rises by 20.7\% and $\rho = 0.907$; 18 of 27 field leaders survive. Carnegie Mellon's Computer Science lead also falls at this threshold, with Stanford leading instead, consistent with the same mechanism of Stanford having a larger population of researchers with at least 20\% CS output, even if CS is not their modal field. The Wageningen Ag\&Bio and Environmental Science results are unchanged from the 33\% case. These findings confirm that the overall field ranking structure is broadly stable under multi-field assignment but that the field-leadership claims for smaller, highly specialised institutions depend on how narrowly ``counts in this field'' is defined. Given this sensitivity, the field-leadership claims in Table~\ref{tab:field-leaders} should be read as reflecting depth of specialisation under the paper's single-field assignment.

\subsection{Robustness of institutional rankings to affiliation-tie assignment}
\label{subsec:results-institution-robustness}
\textcolor{red}{The single-institution assignment described in Subsection~\ref{subsec:author-assignment} breaks ties among an author's equally-most-recent educational institutions by taking the smallest OpenAlex institution identifier. Roughly 3,747,235 of the 29,970,900 qualifying authors (12.5\%) have a genuine tie: two or more educational institutions with an identical latest affiliation year. Re-assigning each such author to every tied institution, following the inclusive multi-membership procedure of Subsection~\ref{subsec:methods-institution-robustness}, expands the (author, institution) roster from 29,970,900 pairs to 35,291,037 (+17.8\%) and adds 271 institutions to the 20,932 that clear the data filters.}

\textcolor{red}{Institution-level h\textsubscript{2} is highly stable under this re-assignment: on the 20,932 institutions shared between the baseline and multi-institution rosters, the Spearman rank correlation is $\rho = 0.9658$, with top-10 overlap of 80\% and top-20 overlap of 70\%. Country-level h\textsubscript{3} is even more stable, as expected since institution$\to$country mapping is a fixed lookup untouched by this re-assignment: $\rho = 0.9965$ across all 214 shared countries, with top-10 overlap of 100\% and top-20 overlap of 95\%.}

\textcolor{red}{Saudi Arabia's efficiency figure, flagged elsewhere in this paper (Subsection~\ref{subsec:h3-efficiency}) as a plausible artefact of funding-driven affiliation listing~\citep{saudi}, moves under this check, but in the wrong direction to be explained by the tiebreak heuristic: $\varepsilon_3$ \emph{rises} from 6.692 to 7.495 once tied authors are counted at every tied institution, rather than falling as would be expected if the arbitrary tiebreak were artificially inflating Saudi institutions' apparent headcount advantage. This rules out the current-affiliation tiebreak mechanism specifically as the source of Saudi Arabia's efficiency anomaly; it does not identify what the true mechanism is, and is consistent with the funding-affiliation explanation already offered elsewhere in the paper, which this check cannot test directly.}

\textcolor{red}{Taken together, these results indicate that the arbitrary tiebreak rule used for single-institution assignment is a comparatively minor contributor to ranking instability: it moves institution rankings by a bounded, measurable amount and country rankings by very little. This does not, however, resolve the broader concern that a researcher's entire career h-index is credited to wherever they are currently affiliated, tied or not; that concern is addressed in this paper by narrowing its claims rather than by further re-attribution (Subsection~\ref{subsec:author-assignment} and the Limitations paragraph of the Conclusion).}
...
The failure of the compounding formula does not, however, undermine the efficiency metric. The two claims play different roles. The compounding formula ($\beta_1 = \alpha_0\alpha_1$) is a \emph{theoretical prediction} about the relationship between separately-fitted exponents; it is tested here and found wanting. The power-law scaling $\mathrm{h}_2 \propto \mathrm{S}^{1/\beta_1}$ is an \emph{empirical finding} ($R^2 = 0.911$) that does not depend on the compounding formula being correct. $\beta_1$ is estimated directly from the distribution of $\mathrm{h}_1$ values across authors, not derived from $\alpha_0\alpha_1$, so the 177\% discrepancy in the compounding formula has no bearing on the exponent actually used in $\varepsilon_2$. This is checked against the Leiden Ranking and, \textcolor{red}{more independently, against Scimago Institutions Rankings,} later in this subsection\textcolor{red}{, though as discussed there, raw h\textsubscript{2} tracks both external rankings more closely than $\varepsilon_2$ does, so those comparisons should be read as description of the efficiency metric's behavior rather than as confirmation of it}.
...
\textcolor{red}{Four of the ten institutions in Table~\ref{tab:h2-efficiency-all} --- Bellevue University, Imperial Valley College, City College of San Francisco, and Frederick Community College --- are primarily-teaching institutions whose reported h\textsubscript{2} is implausible on its face for their type. Manually inspecting each institution's highest-h\textsubscript{1} contributing authors finds the mechanism: rather than genuine small research populations, these authors each list an implausible number of distinct lifetime educational institutions (46--340, against a median of 4--7 across the rest of each institution's roster), spanning unrelated countries, disciplines, and career eras within a single record --- the signature of OpenAlex author-disambiguation merge failure (several distinct researchers collapsed into one author identifier) rather than authors genuinely affiliated with these institutions. Excluding authors with 20 or more distinct lifetime educational institutions and recomputing h\textsubscript{2} for these four institutions in isolation gives 28 (Bellevue, from 54), 55 (Imperial Valley, from 62), and 35 (City College of San Francisco, from 43); Frederick Community College's h\textsubscript{2} is unchanged at 37, since only one of its 416 assigned authors meets this threshold and removing it does not cross the rank-37 value. This substantially closes the gap for three of the four institutions, though not all the way to a value a two-year teaching institution's research output would plausibly support on its own, indicating the merge-artifact pattern is not fully captured by this single threshold. I do not apply this exclusion to the paper's other tables: a systematic evaluation of how many institutions across the full dataset carry the same pattern is beyond the scope of the present revision, and the top of Table~\ref{tab:h2-efficiency-all} should be read with this caveat rather than as a validated efficiency finding.}
...
\subsubsection*{External validation against the Leiden Ranking}
\label{subsubsec:leiden-validation}
Matching the 1,506 Leiden Ranking Open Edition 2024 universities to OpenAlex's dataset yields 1,495 institutions with both an MNCS value and an institution-wide h\textsubscript{2} from OpenAlex. The Spearman rank correlation between raw h\textsubscript{2} and MNCS is $\rho = 0.634$ ($p \approx 6 \times 10^{-170}$); between $\varepsilon_2$ and MNCS it is $\rho = 0.580$ ($p \approx 5 \times 10^{-135}$). \textcolor{red}{Because the Leiden Open Edition is itself built on OpenAlex, this agreement is evidence of convergence within a shared data infrastructure rather than independent validation; Subsubsection~\ref{subsubsec:scimago-validation} below repeats the comparison against a source with no OpenAlex dependency.}

\subsubsection*{External validation against Scimago Institutions Rankings}
\label{subsubsec:scimago-validation}
\textcolor{red}{Joining the Scimago Institutions Rankings (SIR) 2026 Overall Rank export (10,827 institutions, Scopus-derived) to the paper's $\geq$100-author efficiency table (12,026 institutions) on normalized institution name, since the SIR export carries no ROR or OpenAlex identifier, yields 3,659 matched institutions: 33.8\% of SIR's list and 30.4\% of the paper's efficiency table. This is a materially less precise join than the ID-based Leiden match above, so it is read alongside that caveat.}

\textcolor{red}{On the matched set, the Spearman rank correlation between raw h\textsubscript{2} and SIR's Global Rank is $\rho = 0.752$ ($p \approx 0$, $n = 3{,}659$); between $\varepsilon_2$ and SIR's Global Rank it is $\rho = 0.338$ ($p \approx 2 \times 10^{-98}$). Restricting the match to SIR's ``Universities'' sector only (5,491 of SIR's 10,827 institutions, excluding ministries, academies, hospitals, and companies that SIR also ranks) gives essentially the same result ($\rho = 0.755$ raw, $\rho = 0.332$ efficiency), so the gap between the two correlations is not an artefact of comparing an academic h\textsubscript{2} ranking against a mixed-sector SIR list.}

\textcolor{red}{This is the same pattern as the Leiden comparison above, now against a source with no shared data infrastructure with OpenAlex at all: raw h\textsubscript{2} tracks the external ranking (Leiden's MNCS, SIR's Global Rank) more closely than the size-normalized efficiency metric $\varepsilon_2$ does. I report both correlations as description rather than as validation of the efficiency normalization; a plausible mechanistic reading is that external composite rankings like SIR's Global Rank and Leiden's MNCS are themselves influenced by scale-correlated factors (institutional prominence, resource concentration) that raw h\textsubscript{2} partially captures and that dividing out size by construction removes. That both external comparisons show this same direction, using different data sources and different underlying constructs, argues against either result being a coincidence of one particular external ranking.}
...
Table~\ref{tab:h3-efficiency} ranks the 151 countries with at least 5 qualifying institutions by efficiency $\varepsilon_3 = \mathrm{h}_3 / \mathrm{R}^{1/\beta_2}$. The Italy--Japan contrast foreshadowed in Subsection~\ref{subsec:h3-country} is now quantified: Italy ($\varepsilon_3 = 7.35$, rank 1) achieves $\mathrm{h}_3 = 45$ from only 229 institutions, while Japan ($\varepsilon_3 = 4.15$, rank 44) reaches $\mathrm{h}_3 = 46$ from 1,358, virtually the same headline number but at $6{\times}$ the institutional scale. Australia (rank 2, $\varepsilon_3 = 7.29$, 120 institutions) and Spain (rank 3, $\varepsilon_3 = 7.24$, 156 institutions) round out a top tier of countries with \textcolor{red}{compact research systems that place well on this metric}. Germany (rank 6), the United Kingdom (rank 7), and France (rank 8) are the only countries that place in the top 10 on both raw $\mathrm{h}_3$ and efficiency\textcolor{red}{; placing well on both measures simultaneously is a weaker claim than confirming that the underlying research depth is genuine, since neither raw $\mathrm{h}_3$ nor $\varepsilon_3$ is constructed to rule out the kind of affiliation-attribution distortion discussed below for Saudi Arabia}. Saudi Arabia is a notable outlier at 4th by efficiency (h\textsubscript{3}=25 on just 52 institutions) despite ranking only 20th by raw h\textsubscript{3}; as flagged in Subsection~\ref{subsec:methods-crossval}, this is plausibly an artifact of high-h-index researchers listing Saudi institutions as secondary affiliations in exchange for funding~\citep{saudi} rather than a genuine concentration of resident research talent\textcolor{red}{. Saudi Arabia is simply the case where this risk is most visible in the numbers; the multi-institution robustness check in Subsection~\ref{subsec:results-institution-robustness} rules out the current-affiliation tiebreak heuristic specifically as its cause (Saudi Arabia's $\varepsilon_3$ rises, not falls, under that check) but does not establish that other countries' efficiency ranks are free of comparable affiliation-attribution risk, including the possibility of secondary or funding-driven affiliation listing, which this paper cannot test directly}.
...
\begin{table}[h]
	...
	\midrule
	1\%  & 0.999 & 93\% & 0.9997 & 94\%  & 5/5 & 5/5 & 5/5 \\
	5\%  & 0.997 & 84\% & 0.999  & 96\%  & 5/5 & 5/5 & 5/5 \\
	10\% & 0.994 & 78\% & 0.998  & 90\%  & 4/5 & 5/5 & 5/5 \\
	13\%* & 0.993 & 78\% & 0.998  & 90\%  & 4/5 & 5/5 & 5/5 \\
	20\% & 0.991 & 80\% & 0.997  & 90\%  & 3/5 & 4/5 & 3/5 \\
	40\% & 0.988 & 79\% & 0.996  & 90\%  & 5/5 & 4/5 & 5/5 \\
	80\% & 0.988 & 83\% & 0.994  & 90\%  & 5/5 & 5/5 & 5/5 \\
	\bottomrule
	\end{tabular}
	\vspace{2pt}\raggedright *cross-validation sample's raw outlier fraction, included for reference only.
\end{table}