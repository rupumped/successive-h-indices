...
\end{itemize}
All four fall in the non-Anglophone-institution or common-name strata of the sample, matching the concern raised in Subsection~\ref{subsec:methods-crossval} that disambiguation is more error-prone for exactly these authors. Because such merges can only inflate an author's apparent $\mathrm{h}_1$, and $\mathrm{h}_1$ feeds directly into $\mathrm{h}_2$ and $\mathrm{h}_3$, a handful of merged identities at the right institution could measurably distort an institution- or country-level ranking; I flag this as an open limitation rather than one this sample can size definitively. \textcolor{red}{This sample is deliberately adversarial -- chosen to find errors, not to estimate how often they occur -- so the 4/19 (21\%) outlier fraction found here should not be read as an estimate of how much of the full 17.1-million-author dataset is affected. The same pattern, disambiguation precision concentrated lower for common names and non-Anglophone institutions, appears in an independent benchmark of 2{,}700 scholars sampled systematically across three regions~\citep{zhao2025}, corroborating that this is a real, geographically and onomastically concentrated failure mode rather than an artifact of this particular sample; OpenAlex's maintainers report an ongoing correction effort targeting exactly this kind of over-merging, with corresponding-author identification precision rising from 0.60 to 0.92 (F1 0.72 to 0.90) after a 2026 rework of the underlying author-matching model~\citep{demes2026}. Subsection~\ref{subsec:results-sensitivity} quantifies how much a population-wide error rate at or below the adversarial sample's raw rate would actually move this paper's rankings.}
...
\textcolor{red}{Table~\ref{tab:sensitivity} reports, for six simulated population-wide error rates spanning $p=1\%$ to a deliberately implausible $p=80\%$ (Subsection~\ref{subsec:methods-sensitivity}), the Spearman rank correlation and top-set overlap between the perturbed and unperturbed rankings, averaged over five trials, together with how often the paper's two headline field-leadership claims survive the perturbation intact. The cross-validation sample's own raw outlier fraction (4/19 $\approx 21\%$, Subsection~\ref{subsec:results-crossval}) is included as an extra row for reference, though it carries no special status in this analysis.}

\textcolor{red}{Institution- and country-level rank correlations decline only modestly with $p$ and then plateau rather than continuing toward zero: Spearman $\rho$(h\textsubscript{2}) falls from 0.999 at $p=1\%$ to 0.991 at $p=20\%$ and then stays flat at 0.988 all the way out to $p=80\%$; $\rho$(h\textsubscript{3}) shows the same pattern, from 0.9996 down to 0.994. This follows from how h\textsubscript{2} and h\textsubscript{3} are computed: each is an order statistic (the largest rank whose value has not yet fallen below it), so what matters for a given institution or country is whether a noisy author sits near \emph{its own} threshold, not how many noisy authors exist elsewhere in the dataset; and because the perturbation is bounded in magnitude (at most $\approx 4\times$, the largest of the four observed ratios) and applied independently and identically everywhere, raising $p$ further increasingly inflates competing institutions' threshold authors by similar amounts, which cancels out much of the relative shift. There is no error rate in this model, however extreme, at which the aggregate institution and country rankings degrade toward noise.}

\textcolor{red}{The field-specific leadership claims for Wageningen (Agricultural \& Biological Sciences, Environmental Science) and Carnegie Mellon (Computer Science) are noisier, as single-institution comparisons necessarily are with five trials per rate: each survives in 3 to 5 of 5 trials at every rate tested, with no consistent trend toward more or less frequent survival as $p$ increases -- each of the three claims has its single worst outcome (3/5) at a different rate, which reads as small-sample noise rather than a genuine breaking point. Carnegie Mellon's Computer Science lead is the most robust, failing in only one trial across the entire grid; Wageningen's two leads are each knocked out in one trial at some rate, without a clear ordering between them. Italy and Japan's near-parity in h\textsubscript{3} despite a 6$\times$ difference in institutional scale is preserved throughout the tested range: both values rise together under added noise (Italy 45 at $p=1\%$ to 66--67 at $p=80\%$; Japan 46 to 86--88 over the same range), and while their ratio drifts from about 0.96 down to about 0.76 as $p$ increases, even at that extreme it remains more than four times tighter than the underlying 1:6 gap in institutional count.}
...
\begin{table}[h]
	...
	\midrule
	\textcolor{red}{1\%}  & \textcolor{red}{0.999} & \textcolor{red}{90\%} & \textcolor{red}{0.9996} & \textcolor{red}{100\%} & \textcolor{red}{5/5} & \textcolor{red}{5/5} & \textcolor{red}{5/5} \\
	\textcolor{red}{5\%}  & \textcolor{red}{0.997} & \textcolor{red}{87\%} & \textcolor{red}{0.999}  & \textcolor{red}{92\%}  & \textcolor{red}{5/5} & \textcolor{red}{5/5} & \textcolor{red}{5/5} \\
	\textcolor{red}{10\%} & \textcolor{red}{0.994} & \textcolor{red}{81\%} & \textcolor{red}{0.998}  & \textcolor{red}{90\%}  & \textcolor{red}{5/5} & \textcolor{red}{5/5} & \textcolor{red}{5/5} \\
	\textcolor{red}{20\%} & \textcolor{red}{0.991} & \textcolor{red}{81\%} & \textcolor{red}{0.997}  & \textcolor{red}{90\%}  & \textcolor{red}{5/5} & \textcolor{red}{4/5} & \textcolor{red}{5/5} \\
	\textcolor{red}{21\%*} & \textcolor{red}{0.991} & \textcolor{red}{83\%} & \textcolor{red}{0.997}  & \textcolor{red}{90\%}  & \textcolor{red}{3/5} & \textcolor{red}{5/5} & \textcolor{red}{4/5} \\
	\textcolor{red}{40\%} & \textcolor{red}{0.988} & \textcolor{red}{85\%} & \textcolor{red}{0.996}  & \textcolor{red}{90\%}  & \textcolor{red}{4/5} & \textcolor{red}{3/5} & \textcolor{red}{5/5} \\
	\textcolor{red}{80\%} & \textcolor{red}{0.988} & \textcolor{red}{86\%} & \textcolor{red}{0.994}  & \textcolor{red}{90\%}  & \textcolor{red}{5/5} & \textcolor{red}{5/5} & \textcolor{red}{5/5} \\
	\bottomrule
	\end{tabular}
