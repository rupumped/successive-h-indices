...
Two structural features of this procedure limit the impact of field classification errors. First, the paper operates at the \emph{field} level (26 fields) rather than at the finer topic level (${\sim}4{,}500$ topics); OpenAlex's topic classifier fine-tunes a multilingual BERT model on title, abstract, and citation data and achieves a top-1 accuracy of 0.53~\citep{vaneck2024}, but many topic-level errors stay within the same parent field, so field-level accuracy is substantially higher. Second, an author's field is determined by the mode of their \emph{entire publication record}, not by a single work: a researcher would need a substantial fraction of their career output systematically misclassified to a different field before their assignment would change.
