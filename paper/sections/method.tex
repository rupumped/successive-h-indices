...
\textcolor{red}{Because the cross-validation sample above cannot itself bound a population-wide error rate, I separately ask how much author-disambiguation error the paper's conclusions could tolerate before changing, using a Monte Carlo sensitivity analysis run directly on the full authors table. At a population-wide error rate $p$, each author is independently flagged with probability $p$; a flagged author's $\mathrm{h}_1$ is multiplied by an inflation ratio drawn uniformly from the four ratios actually observed among the cross-validation outliers in Subsection~\ref{subsec:results-crossval} (Donald Small $103/57$, Karen S.\ Calhoun $45/11$, Lei Jiang $215/115$, A.\ Jafari $139/41$, all against the lowest external value found for that author), simulating a merged-identity error of the same kind and magnitude found by hand. I recompute institution-level h\textsubscript{2}, country-level h\textsubscript{3}, and the field-specific h\textsubscript{2} rankings underlying the Wageningen and Carnegie Mellon field-leadership claims from Section~\ref{sec:results}, and compare each to the unperturbed baseline via Spearman rank correlation and top-set overlap. Rather than picking a single rate to call a ``worst case'' -- the cross-validation sample's own raw outlier fraction (4/19 $\approx$ 21\%) is not a principled bound, only what one particular 19-author draw happened to find -- I run five independent trials at each of six rates, $p \in \{1\%, 5\%, 10\%, 20\%, 40\%, 80\%\}$, tracing out how ranking stability responds to the assumed amount of error from a plausible rate up to a deliberately implausible one, so that the paper's tolerance for this kind of error is characterized by a curve rather than asserted at a single point.}
