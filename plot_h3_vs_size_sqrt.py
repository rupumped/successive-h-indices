#!/usr/bin/env python3
"""
Scatter plot of H3 vs sqrt(institution_count) with a linear trendline and R² value.

Usage:
  python3 plot_h3_vs_size_sqrt.py
"""
...
df["sqrt_inst"] = np.sqrt(df["institution_count"])

x = df["sqrt_inst"].values
y = df["h3"].values
...
for _, row in df.iterrows():
    ax.annotate(row["country_code"], (row["sqrt_inst"], row["h3"]),
...
ax.set_xlabel("√(institution count)", fontsize=12)
