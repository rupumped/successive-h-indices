...
# tests predicts h2 = c*sqrt(N) with no additive constant (N=0 authors
# trivially gives h2=0), so fit that one-parameter model rather than an OLS
# line with a free intercept. R² here is the uncentered version appropriate
# for a no-intercept fit (relative to sum(y^2), not sum((y-mean(y))^2)).
